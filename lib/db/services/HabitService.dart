...
class HabitService {
  ...
  Future<List<Map<String, Object?>>> retrieveHabitsOfUser(User user) async {
    ...
    List<Map<String, Object?>> habits =
        await db.rawQuery('SELECT * FROM habits WHERE user_id=?', [userId]);
    return habits;
  }

  Future<List<Map<String, Object?>>> retrieveHabitsOfFriend(
      Friend friend) async {
    final db = await _dbHelper.database;

    int? friendId = friend.friend_id;
    List<Map<String, Object?>> habits =
        await db.rawQuery('SELECT * FROM habits WHERE friend=?', [friendId]);
    return habits;
  }
}
